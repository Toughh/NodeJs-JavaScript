...
describe('PATCH users API (-ve) - Verify user error codes and messages *end-to-end*', function () {
    ...
    it('C38836: PATCH > Users > Validate ERROR when patching an users API not belonging to subadmins group', async function () {
        const expectedErrorMes = {
            "error_code": "GEN010",
            "developer_message": "Request method 'PATCH' not supported",
            "user_message": "Unsupported request."
        }

        res = await requests.patchUsersApiUsersFaxNumber(fax_number1, app_id1, userName)
    ...
    it('C38838: PATCH > Users > Validate ERROR when patching an users API not belonging to subadmins subgroup', async function () {
        const expectedErrorMes = {
            "error_code": "GEN010",
            "developer_message": "Request method 'PATCH' not supported",
            "user_message": "Unsupported request."
        ...
        res = await requests.patchUsersApiUsersFaxNumber(fax_number2, app_id2, userName)
        assert.equal(res.statusCode, 400, 'Actual status code is ' + res.statusCode)
        assert.include(res.body.errors[0], expectedErrorMes, 'Actual Error Message is ' + res.body.errors[0])
    })
