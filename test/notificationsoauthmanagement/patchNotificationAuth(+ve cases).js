...
describe('PATCH notifications Auth (+) - Verify response *end-to-end*', function () {
    ...
    let client_id, client_secret, token_endpoint, grant_type, scope, updatedDescription, UpdatedEnabled
    ...
    before(async function () {
        try {
            ...
            notificationAuthRequestBody.description = common_utils.randString('Updated Description', 6)
            updatedDescription = notificationAuthRequestBody.description
            UpdatedEnabled = notificationAuthRequestBody.enabled
    ...
    it('C28054: PATCH Notifications auth > Validate notification auth data is updated *smoke*', async function () {
        ...
        assert.equal(res.body.description, updatedDescription, 'Generated description is not same as notifications auth payload description')
        assert.equal(res.body.enabled, UpdatedEnabled, 'Actual enabled boolean value is '+res.body.enabled)
