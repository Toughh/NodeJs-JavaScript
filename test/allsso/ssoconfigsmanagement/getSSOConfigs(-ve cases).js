...
describe('GET /sso/configs/sso_config_id (-) - Validate response *sso* *end-to-end*', function () {
    ...
    it('C31267: GET > SSO > Config > Internal > sso_config_id is inactive > Validate 404 Not Found error is returned', async function () {
    ...
    it('C31268: GET > SSO > Config > Internal > sso_config_id does not exist > Validate 404 Not Found error is returned', async function () {
        res = await requests.getSSOConfigs(randGuid)
        assert.equal(res.status, 404, 'Actual status is ' + res.status)
        assert.equal(res.body.errors[0].error_code, 'SSO002', 'Actual error_code is '+res.body.errors[0].error_code)
