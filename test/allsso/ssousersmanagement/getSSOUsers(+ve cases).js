...
describe('GET /sso/users (+) - Validate response *sso* *end-to-end*', function () {
    ...
    let group_name, app_id, fax_number, sso_config_id, customer_key, email, oauth_group, client_id, sso_name_id, sso_user_id
    ...
    it('C31156: GET > SSO > Users > Internal > Validate SSO user details are returned', async function () {
        res = await requests.getSSOUsers(sso_user_id)
        assert.equal(res.status, 200, 'Actual status is ' + res.status)
        assert.equal(res.body.sso_user_id, sso_user_id, 'Actual sso_user_id is '+res.body.sso_user_id)
    ...
    it('C32033: GET > SSO > Users > customer_key > Internal > Validate SSO user details are returned if user has existing customer_key and is sso user', async function () {
        res = await requests.getUsersCustomerKeySSO(customer_key)
        assert.equal(res.status, 200, 'Actual status is ' + res.status)
