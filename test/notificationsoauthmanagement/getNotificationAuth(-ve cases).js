...
describe('GET notifications Auth (-) - Verify response *end-to-end*', function () {
    ...
    it('C28062: GET Notifications auth > Validate no notification auths returned passing not existing app-id *smoke*', async function () {
        res = await requests.getApplicationsNotificationsAuth(randGuid, notify_id)
    ...
    it('C28063: GET Notifications auth > Validate no notification auths returned passing not existing notification id', async function () {
        res = await requests.getApplicationsNotificationsAuth(app_id, randGuid)
        assert.equal(res.status, 404, 'Actual status is '+res.status)
    ...
    it('C28065: GET Notifications auth > Validate notification auth NOT returned passing not existing app-id', async function () {
        res = await requests.getApplicationsNotificationsAuth(randGuid, notify_id)
        assert.equal(res.status, 404, 'Actual status is '+res.status)
        assert.equal(res.body.errors[0].error_code, 'NAUT003', 'Actual error_code is ' + res.body.errors[0].error_code)
        assert.equal(res.body.errors[0].element_name, 'ValidationResult', 'Actual element_name is ' + res.body.errors[0].element_name)
    ...
    it('C28066: GET Notifications auth > Validate no notification auths returned passing not existing notification id', async function () {
        res = await requests.getApplicationsNotificationsAuth(app_id, randGuid)
        assert.equal(res.status, 404, 'Actual status is '+res.status)
        assert.equal(res.body.errors[0].error_code, 'NAUT011', 'Actual error_code is ' + res.body.errors[0].error_code)
        assert.equal(res.body.errors[0].element_name, 'ValidationResult', 'Actual element_name is ' + res.body.errors[0].element_name)
    ...
    it('C28067: GET Notifications auth > Validate notification auth NOT returned passing not existing notification auth-id', async function () {
        res = await requests.getApplicationsNotificationsAuth(app_id, randGuid)
        assert.equal(res.status, 404, 'Actual status is '+res.status)
        assert.equal(res.body.errors[0].error_code, 'NAUT011', 'Actual error_code is ' + res.body.errors[0].error_code)
        assert.equal(res.body.errors[0].element_name, 'ValidationResult', 'Actual element_name is ' + res.body.errors[0].element_name)
